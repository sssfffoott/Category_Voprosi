...
class AdminPanel:
    ...
    def show_menu(self):
        ...
        card.pack(pady=40, padx=40, ipadx=40, ipady=30)
    ...
    def reset_categories(self):
        """Сброс всех тем к встроенным"""
        if messagebox.askyesno("ПОДТВЕРЖДЕНИЕ", 
                               "Это действие удалит все существующие темы и восстановит стандартные (Python, Алгоритмы, Базы данных по 10 вопросов).\n\nПродолжить?"):
    ...
    def add_category(self):
        ...
        container = tk.Frame(self.app.frame, bg=self.app.colors['bg'])
        container.pack(expand=True, fill=tk.BOTH, padx=40, pady=30)
        
        card = self.app.create_card(container)
        card.pack(fill=tk.BOTH, expand=True, ipadx=30, ipady=20)
        
        tk.Label(card, text="[ ДОБАВЛЕНИЕ ТЕМЫ ]", font=self.app.fonts['subtitle'],
                fg=self.app.colors['primary'], bg=self.app.colors['card_bg']).pack(pady=(0, 20))
        ...
        save_btn = tk.Button(btn_frame, text="[ СОХРАНИТЬ ]", command=save,
                            ...
                            activebackground=self.app.colors['success'], activeforeground='#000000')
        save_btn.pack()
        save_btn.bind("<Enter>", lambda e: save_btn.configure(bg=self.app.colors['primary'], fg='#000000'))
        save_btn.bind("<Leave>", lambda e: save_btn.configure(bg=self.app.colors['success'], fg='#000000'))
    ...
    def edit_category(self):
        ...
        def save():
            ...
            messagebox.showinfo("УСПЕХ", f"Тема '{new_name}' обновлена! ({len(questions)} вопросов)")
            self.show_menu()
    ...
    def show_all_results(self):
        ...
        if not all_results:
            ...
            style = ttk.Style()
            style.configure("Treeview", background=self.app.colors['dark'], 
