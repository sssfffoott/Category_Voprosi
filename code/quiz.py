...
class Quiz:
    ...
    def show_results(self):
        ...
        tk.Button(btn_frame, text="Новое тестирование",
                  command=self.restart_quiz, font=('Arial', 14), bg='lightgreen'
                  ).pack(side=tk.LEFT, padx=(0, 10))
        tk.Button(btn_frame, text="Посмотреть ответы",
                  command=self.show_answers, font=('Arial', 14), bg='lightblue'
                  ).pack(side=tk.LEFT, padx=(0, 10))
        tk.Button(btn_frame, text="Главное меню", command=self.go_to_main_menu,
    ...
    def show_answers(self):
        ...
        text_area = tk.Text(win, font=('Arial', 11), wrap=tk.WORD)
