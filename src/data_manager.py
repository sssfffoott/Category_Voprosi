...
class DataManager:
    ...
    # ================= RESULTS =================
    def add_result(self, username, category, score, total):
        """Добавить результат пользователю"""
        percent = round((score / total) * 100, 1)

        for user in self.users:
            if user.username == username:
                user.add_result(category, score, total, percent)
                self.save_users()
                return

        new_user = User(username)
        new_user.add_result(category, score, total, percent)
        self.users.append(new_user)
        self.save_users()
    ...
    def reset_to_sample_data(self):
        if os.path.exists('categories'):
            for filename in os.listdir('categories'):
                if filename.endswith('.json'):
                    os.remove(os.path.join('categories', filename))
        else:
            os.makedirs('categories')
