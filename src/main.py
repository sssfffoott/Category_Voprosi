...
class QuizApp:
    def __init__(self, root):
        ...
        self.root.minsize(800, 600)  # минимальный размер, чтобы ничего не обрезалось
    ...
    def show_role_selection(self):
        ...
        # Центрируем содержимое с помощью place
        container = tk.Frame(self.frame, bg=self.colors['bg'])
        container.place(relx=0.5, rely=0.5, anchor='center')
        
        title_frame = tk.Frame(container, bg=self.colors['bg'])
